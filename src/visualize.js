 ...
 * @param options
 */
export default function visualize(re,flags,paper, options) {
...
var plotNode={
  ...
  dot:function (node,x,y) {
    ...
    var a=textRect(locale.AnyCharExceptNewLine,x,y,bgColor,textColor);
    a.rect.r=10;
    a.rect.tip= locale.AnyCharExceptCRLF
    return a;
  },
  ...
  charset:function (node,x,y) {
    ...
    if (simple) {
      ...
      if (!node.exclude) {
        ...
        var tl=textLabel(a.x+a.width/2,a.y,locale.NoneOf,labelColor);
        var items=a.items;
        items.push(tl.label);
  ...
  group:function (node,x,y) {
    ...
    if (node.num) {
      ...
      var tl=textLabel(rect.x+rect.width/2,rect.y-strokeWidth,`${locale.group} #`+node.num);
